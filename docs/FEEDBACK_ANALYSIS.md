...
| F28 | 09-10 | A | **Drive, where necessary**: "I should get enough help and drive so that I do what is good to do — for my life, for others, for the world." Witnessing alone is not enough; the app should actively propose the next right action | strategy | recorded — shapes Steward (2.3) + briefing: propose **one** next right action with a reason; drive = sharper proposals, not more notifications (ahimsa intact) | ROADMAP 2.3, design §0 |
| F27 | 09-10 | A | Logged out → sign in has **no Forgot password**; returning users stuck if they forget credentials | ux/bug | **shipped** (1.7a, 2026-09-10) | ROADMAP 1.7a |
...
| 2026-09-10 | **F27:** no Forgot password on sign-in (`AuthGate.tsx`); owner blocked when logged out. Next build: `resetPasswordForEmail` + "Check your email" state; confirm Supabase redirect URL for mobile. |
